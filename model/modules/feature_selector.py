...
class SparseSync(torch.nn.Module):
    ...
    def __init__(
        ...
        self.v_selector = instantiate_from_config(
            v_selector_cfg
        )  # FeatureSelectorTransformer
        self.a_selector = instantiate_from_config(
            a_selector_cfg
        )  # FeatureSelectorTransformer
        # aggregation transformer
        self.global_transformer = instantiate_from_config(
            global_transformer_cfg
        )  # GlobalTransformer
        # head
        if isinstance(self.global_transformer, GlobalMLP):
            N_av = (
                a_selector_cfg.params.num_selectors
                + v_selector_cfg.params.num_selectors
            )
            self.off_head = torch.nn.Linear(
                N_av * self.config.n_embd, self.config.num_offset_cls
            )
        else:
            self.off_head = torch.nn.Linear(
                self.config.n_embd, self.config.num_offset_cls, bias=False
            )
    ...
    def forward(self, vis: torch.Tensor, aud: torch.Tensor):
        ...
        vis, aud = (
            vis.permute(0, 1, 3, 4, 2).contiguous(),
            aud.permute(0, 2, 3, 1).contiguous(),
        )
...
class FeatureSelectorTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.pos_emb_cfg is not None:
            ...
            logger.info(
                f'Positional embedding module for selectors: {pos_emb_cfg.target.split(".")[-1]}'
            )
        ...
        self.blocks = torch.nn.Sequential(
            *[CrossBlock(config) for _ in range(config.n_layer)]
        )
    ...
    def forward(self, context):
        ...
        # maybe add pos emb
        selectors = (
            selectors if self.pos_emb_cfg is None else self.selectors_pos_emb(selectors)
        )
        # dropout
...
class GlobalTransformer(torch.nn.Module):
    ...
    def __init__(
        self, tok_pdrop, embd_pdrop, resid_pdrop, attn_pdrop, n_layer, n_head, n_embd
    ) -> None:
        super().__init__()
        self.config = Config(
            embd_pdrop=embd_pdrop,
    ...
    def forward(
        self, vis_selectors: torch.Tensor, aud_selectors: torch.Tensor, targets=None
    ):
        ...
        # norm
        vis_selectors, aud_selectors = self.vis_in_lnorm(
            vis_selectors
        ), self.aud_in_lnorm(aud_selectors)
        # maybe whole token dropout
        vis_selectors, aud_selectors = self.tok_drop_vis(
            vis_selectors
        ), self.tok_drop_aud(aud_selectors)
        # (B, 2+Sv+1+Sa, D)
...
class CrossAttention(SelfAttention):
    ...
    def forward(self, x, context):
        ...
        q = (
            self.query(x).view(B, Tx, self.n_head, C // self.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
        k = (
        ...
        v = (
            self.value(context)
            .view(B, Tc, self.n_head, C // self.n_head)
            .transpose(1, 2)
        )  # (B, nh, T, hs)
        ...
        y = (
            y.transpose(1, 2).contiguous().view(B, Tx, C)
