...
class AudioResampleDynamic(torch.nn.Module):
    ...
    def forward(self, item):
        ...
        if a_fps_orig != self.new_freq:
            logger.warning(
                f'Video: {item["path"]} has framerate: {a_fps_orig} changing to {self.new_freq}'
            )
            item["audio"] = torchaudio.functional.resample(
                item["audio"], a_fps_orig, self.new_freq
            )
...
class RGBSpatialCropSometimesUpscale(torch.nn.Module):
    ...
    def __init__(
        self, sometimes_p, target_input_size, is_random, smaller_input_size=None
    ):
        super().__init__()
...
class RandomApplyColorDistortion(torch.nn.Module):
    def __init__(self, p_gray_scale=0.0, p_color_jitter=0.0, s=1.0) -> None:
        ...
        rand_color_jitter = torchvision.transforms.RandomApply(
            [color_jitter], p_color_jitter
        )
...
def make_class_grid(leftmost_val, rightmost_val, grid_size, grid_type="linspace"):
    assert grid_size >= 3, f"grid_size: {grid_size} <= 3"
    if grid_type == "linspace":
        ...
        grid = np.linspace(
            leftmost_val - cell_mid_point, rightmost_val + cell_mid_point, grid_size + 2
        )[1:-1]
        return torch.from_numpy(grid).float()
    elif grid_type == "uniform_2cls":
        assert grid_size % 4 == 0, f"grid_size: {grid_size} % 4 != 0"
        zeros = np.zeros(grid_size // 2)
        left = np.linspace(leftmost_val, 0, grid_size // 4 + 1)[:-1]
        right = np.linspace(0, rightmost_val, grid_size // 4 + 1)[1:]
        grid = np.concatenate([left, zeros, right])
        return torch.from_numpy(grid).float()
...
class TemporalCropAndOffsetRandomFeasible(TemporalCropAndOffset):
    ...
    def apply_a_jitter(self, a_start_i, max_a_start_i, a_fps):
        max_a_jitter_i = sec2frames(
            self.max_a_jitter_sec, a_fps
        )  # not in self, as a_fps may be dynamic
        ...
        assert (
            0 <= a_start_i <= max_a_start_i
        ), f"{a_jitter_i} {max_a_jitter_i_left} {max_a_jitter_i_right} {max_a_start_i}"
    ...
    def forward(self, item):
        ...
        if self.do_offset:
            ...
            if offset_sec is None and v_start_i_sec is None:
                ...
                if self.max_a_jitter_sec is not None and self.max_a_jitter_sec > 0:
                    ...
                    a_start_i, a_jitter_i = self.apply_a_jitter(
                        a_start_i, max_a_start_i, a_fps
                    )
        ...
        assert (
            item["video"].shape[0] == v_fps * self.crop_len_sec
        ), f'{item["video"].shape} {item["path"]}'
        assert (
            item["audio"].shape[0] == a_fps * self.crop_len_sec
...
class FreezeFrames(torch.nn.Module):
    ...
    def __init__(self, max_off_sec, active_min_overlap_sec, to_freeze_frames) -> None:
        ...
        if to_freeze_frames:
            print(
                f"Going to freeze all frames except for {self.sec_v_is_active} sec randomly"
            )
    ...
    def forward(self, item):
        ...
        assert len(frozen_left) + end_frame - start_frame + len(frozen_right) == len(
            vid
...
class CorruptAudio(torch.nn.Module):
    ...
    def __init__(
        self, max_off_sec, active_min_overlap_sec, to_corrupt_audio, corrupt_type
    ) -> None:
        super().__init__()
        self.max_off_sec = max_off_sec
        self.active_min_overlap_sec = active_min_overlap_sec
        ...
        if to_corrupt_audio:
            assert corrupt_type in [
                "rand",
                "mute",
            ], f"corrupt type {corrupt_type} is not supported"
            print(
                f"Going to corrupt audio except for {self.sec_is_active} sec randomly"
            )
    ...
    def forward(self, item):
        ...
        if self.corrupt_type == "mute":
            ...
            frozen_right = (
                torch.rand(a_len_frames - end_frame, dtype=aud.dtype) * 2 - 1
            )  # to [-1, 1]
...
class AudioTrimOrTileToDuration(torch.nn.Module):
    ...
    def __init__(self, duration_sec: int = 10) -> None:
        ...
        assert duration_sec == int(
            duration_sec
        ), "please use int as duration_sec, otherwise change tiling"
...
class AudioTimeCrop(torch.nn.Module):
    ...
    def forward(self, item):
        ...
        a_len_frames = aud.shape[0]
        a_fps = int(item["meta"]["audio"]["framerate"][0])
        assert a_fps == item["meta"]["audio"]["framerate"][0], (
            f"{a_fps} " + item["meta"]["audio"]["framerate"][0]
        )
        a_crop_len_frames = sec2frames(self.crop_len_sec, a_fps)
        if self.is_random:
    ...
    def forward(self, item):
        if self.p > torch.rand(1):
            ...
            wave = torchaudio.functional.lowpass_biquad(
                wave, sr, self.cutoff_freq, self.Q
            )
...
class AudioRandomReverb(torch.nn.Module):
    ...
    def forward(self, item):
        if self.p > torch.rand(1):
            sr = int(item["meta"]["audio"]["framerate"][0])
            wave = item["audio"].unsqueeze(0)
            item["audio"], _ = torchaudio.sox_effects.apply_effects_tensor(
                wave, sr, self.effects
            )
...
if __name__ == "__main__":
    grid = make_class_grid(-1, 1, 4, grid_type="uniform_2cls")
    grid = make_class_grid(-2, 2, 24, grid_type="uniform_2cls")
    print("grid:", grid)
    print("value quantization:", quantize_offset(grid, 0.1))
    # v_fps = 25.0
    # duration = 9.0

    # input = {
    #     "video": torch.randint(
    #         0, 256, (int(duration * v_fps), 3, 720 // 2, 1280 // 2), dtype=torch.uint8
    #     ),
    ...
    # fn = TemporalCropAndOffsetRandomFeasible(
    #     crop_len_sec=5,
    #     max_off_sec=2,
    #     do_offset=True,
    #     grid_size=3,
    ...
    # )
    # fn = TemporalCropAndOffsetRandomFeasible(
    #     crop_len_sec=5, max_off_sec=2, do_offset=True, grid_size=3, grid_type="linspace"
    # )
    # input = fn(input)
    # print(input["audio"].shape, input["video"].shape, input["meta"]["audio"])
    # print(input["targets"])
