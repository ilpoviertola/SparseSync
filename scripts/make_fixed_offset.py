...
def main(cfg: DictConfig):
    ...
    for t_cfg in transform_sequence_cfg:
        ...
        if t_cfg["target"].endswith(
            (
                ".EqualifyFromRight",
                ".RGBSpatialCropSometimesUpscale",
                ".TemporalCropAndOffsetRandomFeasible",
            )
        ):
    ...
    datasets = get_datasets(
        cfg, {k: data_transforms for k in ["train", "valid", "test"]}
    )
    ...
    for phase in ["valid", "test", "train"]:
        ...
        fixed_offset_path = os.path.join(
            f"./data/fixed_offsets{suffix}", fixed_offset_fname
        )
        with open(fixed_offset_path, "w") as outf:
            ...
            for p, o, s in zip(paths, offs, vstarts):
                writer.writerow(
                    [_get_unique_id(p), round(o.item(), 2), round(s.item(), 2)]
                )
