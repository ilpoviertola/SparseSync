...
def broadcast_obj(object, global_rank, device):
    if dist.is_initialized():
        objects = [
            object if is_master(global_rank) else None
...
def get_loaders(cfg, datasets, batch_sizes):
    train_sampler = (
        DistributedSampler(datasets["train"], shuffle=True)
        if dist.is_initialized()
        else None
    )
    valid_sampler = (
        DistributedSampler(datasets["valid"], shuffle=False)
        if dist.is_initialized()
        else None
    )
    test_sampler = (
        DistributedSampler(datasets["test"], shuffle=False)
        if dist.is_initialized()
        else None
    )
    ...
    subset_size = 0.2
    val_subset = Subset(
        datasets["valid"], torch.randperm(int(len(datasets["valid"]) * subset_size))
    )
    valid_rand_aud_sampler = (
        DistributedSampler(val_subset, shuffle=False) if dist.is_initialized() else None
    )
    valid_rand_rgb_sampler = (
        DistributedSampler(val_subset, shuffle=False) if dist.is_initialized() else None
    )
    valid_perm_batch_sampler = (
        DistributedSampler(val_subset, shuffle=False) if dist.is_initialized() else None
    )
    return {
        "train": DataLoader(
        ...
        "valid_rand_aud": DataLoader(
            ...
            batch_sizes["test"],
            shuffle=False,
            sampler=valid_rand_aud_sampler,
            num_workers=cfg.training.num_workers,
            pin_memory=True,
        ),
        "valid_rand_rgb": DataLoader(
            ...
            batch_sizes["test"],
            shuffle=False,
            sampler=valid_rand_rgb_sampler,
            num_workers=cfg.training.num_workers,
            pin_memory=True,
        ),
        "valid_perm_batch": DataLoader(
            ...
            batch_sizes["test"],
            shuffle=False,
            sampler=valid_perm_batch_sampler,
            num_workers=cfg.training.num_workers,
            pin_memory=True,
        ),
    }
...
def get_optimizer(cfg, model, num_gpus):
    ...
    if cfg.training.optimizer.name == "adam":
        ...
        raise NotImplementedError(
            f'Optimizer: "{cfg.training.optimizer.name}" is not implemented'
        )
    return optimizer
        ...
        lr_sched = lr_scheduler.SequentialLR(
            ...
            schedulers=[
                lr_scheduler.LinearLR(
                    optimizer, start_factor=1 / 100, total_iters=warmup
                ),
...
def load_ckpt(cfg, model_wo_ddp, optimizer, scaler, lr_scheduler):
    ...
    if cfg.training.finetune:
        ...
        new_vid_dir = (
            f"{Path(cfg.data.vids_path).stem}/{Path(cfg.data.vids_path).parent.stem}"
        )
        assert old_vid_dir != new_vid_dir, f"old: {old_vid_dir}; new: {new_vid_dir}"
        assert (
            ckpt_cfg.data.dataset.target != cfg.data.dataset.target
        ), ckpt_cfg.data.dataset.target
        logger.info(
            f"Finetuning from: {ckpt_cfg.ckpt_path} on {cfg.data.dataset.target}"
        )
        ckpt["metrics"][cfg.training.metric_name] = (
            0 if cfg.training.to_max_metric else float("inf")
        )
        start_epoch = 0
...
class EarlyStopper(object):
    ...
    def decide(self, global_rank, logger, metrics):
        ...
        if is_master(global_rank):
            logger.print_logger.info(
                f"decide inputs: {new_metric:.6f}; best: {self.best_metric}"
            )
    ...
    def is_new_model_better_than_curr(self, new_metric_val: float):
        return (
            self.best_metric < new_metric_val
            if self.to_max
            else self.best_metric > new_metric_val
        )
...
def prepare_inputs(batch, device, phase=None):
    ...
    targets = {
        k: targets[k].to(device)
        for k in ["vggsound_target", "offset_target"]
        if k in targets
    }
    ...
    if phase == "valid_rand_aud":
        aud = (aud.max() - aud.min()) * torch.rand(
            aud.shape, dtype=aud.dtype, device=aud.device
        ) + aud.min()
    elif phase == "valid_rand_rgb":
        vid = (vid.max() - vid.min()) * torch.rand(
            vid.shape, dtype=vid.dtype, device=vid.device
        ) + vid.min()
...
def verbose_epoch_progress(global_rank, logger, running_results, phase, epoch):
    ...
    # logging loss values
    if is_master(global_rank):
        logger.log_epoch_loss(
            running_results["loss_total"], epoch, phase, prefix="total"
        )
    # logging metrics
...
def calc_metrics(
    targets, outputs: torch.FloatTensor, topk=(1, 5), only_accuracy=False, prefix=""
):
    """
    ...
    # accuracy@k_tol
    if (
        num_cls == 3 and dataset_size > 100
    ):  # 100 is to avoid verbosity on iteration level
        logger.warning(
            "Accuracy with tolerance is not reliable as num of offset classes is 3."
        )
    ...
    targets_for_acc_w_tol = torch.stack(
        [targets_for_acc_left_tol, targets_for_acc, targets_for_acc_right_tol]
    )
    ...
    # one-vs-rest
    avg_p = [
        average_precision_score(targets[:, c], targets_pred[:, c], average=None)
        for c in range(num_cls)
    ]
    try:
        roc_aucs = [
            roc_auc_score(targets[:, c], targets_pred[:, c], average=None)
            for c in range(num_cls)
        ]
    except ValueError:
        logger.warning(
            "Weird... Some classes never occured in targets. Do not trust the metrics."
        )
    ...
    metrics_dict[f"{prefix}dprime"] = scipy.stats.norm().ppf(
        metrics_dict[f"{prefix}mROCAUC"]
    ) * np.sqrt(2)
