...
from dataset.greatesthit import GreatestHitAudioOnlyDataset as GreatestHitAudioOnly
...
def train(cfg):
    ...
    if transform_sequence_train_cfg is None:
        ...
        transforms_train = [
            instantiate_from_config(c) for c in transform_sequence_train_cfg
        ]
    ...
    if transform_sequence_test_cfg is None:
        ...
        transforms_test = [
            instantiate_from_config(c) for c in transform_sequence_test_cfg
        ]
    ...
    datasets = {
        "train": GreatestHitAudioOnly(
            split="train",
            split_dir_path=Path(splits_path),
            data_path=Path(cfg.data.vids_path),
            meta_path=Path(meta_path),
            transforms=transforms_train,
        ),
        "valid": GreatestHitAudioOnly(
            split="train",
            split_dir_path=Path(splits_path),
            data_path=Path(cfg.data.vids_path),
            meta_path=Path(meta_path),
    ...
    model = ResNetAudio(
        cfg.model, num_classes=cfg.training.num_classes, extract_features=False
    )
    ...
    for epoch in range(start_epoch, num_epochs):
        for phase in ["train", "valid"]:
            ...
            for i, batch in enumerate(prog_bar):
                ...
                # iter logging
                if i % 50 == 0:
                    logger.log_iter_loss(
                        loss.item(), epoch * len(loaders[phase]) + i, phase
                    )
    ...
    logger.print_logger.info(
        (f'The model was trained for {ckpt["epoch"]} epochs. Loss: {ckpt["loss"]:.4f}')
    )
