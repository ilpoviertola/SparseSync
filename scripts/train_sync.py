...
from scripts.train_utils import (
    ...
    set_seed,
    toggle_mode,
    verbose_epoch_progress,
    verbose_iter_progress,
    verbose_lr,
...
def train(cfg):
    ...
    early_stopper = EarlyStopper(
        cfg.training.patience, cfg.training.to_max_metric, cfg.training.metric_name
    )
    ...
    if cfg.training.run_test_only or cfg.training.resume or cfg.training.finetune:
        start_epoch, early_stopper.best_metric = load_ckpt(
            cfg, model_without_ddp, optimizer, scaler, lr_scheduler
        )
    else:
        start_epoch = 0
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if "run_corrupted_val" not in cfg.training or cfg.training.run_corrupted_val:
            phases_to_run_on.extend(
                ["valid_rand_aud", "valid_rand_rgb", "valid_perm_batch"]
            )
        ...
        for phase in phases_to_run_on:
            ...
            if (
                phase == "valid"
                and "VGGSoundSparsePicked" in loaders[phase].dataset.__class__.__name__
            ):
            ...
            for it in range(iter_times):
                ...
                for i, batch in enumerate(prog_bar):
                    ...
                    with torch.set_grad_enabled(phase == "train"):
                        with torch.autocast(
                            "cuda", enabled=cfg.training.use_half_precision
                        ):
                            # saves recontructed input to the model during the first iteration (detects bugs)
                            if (
                                is_master(global_rank)
                                and iter_step == 0
                                and phase in ["train", "valid"]
                            ):
                    ...
                    if phase == "train":
                        make_backward_and_optim_step(
                            cfg, loss, model, optimizer, scaler, lr_scheduler
                        )
                    ...
                    if is_master(global_rank):
                        verbose_iter_progress(
                            logger, prog_bar, iter_step, iter_results, phase
                        )
                        if phase == "train":
                            verbose_lr(
                                logger,
                ...
                if is_master(global_rank):
                    logger.print_logger.info(
                        f"({phase}) Done {it} iterations out of {iter_times}"
                    )

            # logs epoch metrics to tensorboard/wandb
            epoch_loss, metrics = verbose_epoch_progress(
                global_rank, logger, running_results, phase, epoch
            )
        ...
        if early_stopper.triggered:
            if is_master(global_rank):
                logger.print_logger.info(
                    f"Training is early stopped @ {epoch}; RANK: {global_rank}"
                )
            break
    ...
    ckpt_epoch, best_metric_val = load_ckpt(
        cfg, model_without_ddp, optimizer, scaler, lr_scheduler
    )
    if is_master(global_rank):
